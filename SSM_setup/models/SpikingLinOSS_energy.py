...
def apply_hrf_im(A_diag, B, input_sequence, step, a1, a2, uv_learnable=True):
...
def apply_hrf_imex_im_b(A_diag, G,  B, input_sequence, step, self_a1, self_a2, uv_learnable):
    """Compute the LxH output of LinOSS-IM given an LxH input.
    Args:
        A_diag  (float32):   diagonal state matrix   (P,)
        B       (complex64): input matrix            (P, H)
    ...
    if uv_learnable:
        F1 = M_IM_11 * Bu_elements * step + M_IM_11 * self_a1 + M_IM_12 * self_a2
        F2 = M_IM_21 * Bu_elements * step + M_IM_21 * self_a1 + M_IM_22 * self_a2
    else:
        F1 = M_IM_11 * Bu_elements * step
        F2 = M_IM_21 * Bu_elements * step
    F = jnp.hstack((F1, F2))
...
class SpikingLinOSSLayer(eqx.Module):
    ...
    def __call__(self, input_sequence, spikes):
        ...
        if self.discretization == 'IMEX':
            ...
            A_diag = nn.relu(self.A_diag)
            ys = apply_hrf_im(A_diag, B_complex, input_sequence, steps, self.a1, self.a2, uv_learnable=True)
            ...
            ys = apply_hrf_imex_im_b(A_diag, G, B_complex, input_sequence, steps, self.a1, self.a2, uv_learnable=False)
        else:
...
class SpikingLinOSS(eqx.Module):
    ...
    def __call__(self, x, state, key):
        ...
        if self.classification:
            ...
            spikes += jnp.mean(x) * self.linear_layer.linear.weight.shape[0] * self.linear_layer.linear.weight.shape[1]  # scale by total number of elements to keep in line with other activations
            spikes = spikes * x.shape[0]  # scale by batch size to keep in line with other activations
...
class LILinear(eqx.Module):
  ...
  def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
    ...
    if self.kernel is not None:
        ...
        # Causal convolution
        x_proj = jax.lax.conv_general_dilated(
        x_proj,
        self.kernel,
        window_strides=(1,),
...
if __name__ == "__main__":
    ...
    model = SpikingLinOSS(**model_args)

    # # Real model analysis post training
    loaded_model = eqx.tree_deserialise_leaves("outputs/SpikingLinOSS_IM/EigenWorms/T_1.00_time_False_nsteps_100000_lr_0.001_num_blocks_2_hidden_dim_128_vf_depth_None_vf_width_None_ssm_dim_64_ssm_blocks_None_dt0_None_scale_0_lambd_None_seed_2345/final_model.eqx",
                                                model)
    # print(loaded_model.linear_layer.kernel._value)
    # print(loaded_model)
    ...
    data_dir = "data_dir"
    use_presplit = False
    dataset_name = "EigenWorms" # from 1 to 4
    output_step = 5
    ...
    stepsize = 1  # keep this in int only
    ...
    # Example: constant learning rate scheduler
    def lr_scheduler(lr):
        return lr
