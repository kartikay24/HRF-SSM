...
from models.SpikingLinOSSwBN import SpikingLinOSSwBN
from models.SpikingLinOSSwGeLU import SpikingLinOSSwGeLU
from models.SpikingLinOSS_Heterogeneity import SpikingLinOSSwHeterogeneity
...
from models.SpikingLinOSS_energy import SpikingLinOSS
    ...
    *,
    ...
    if model_name == "ncde":
        ...
        ssm = SpikingLinOSS(
            ...
            classification,
            output_step,
            discretization=linoss_discretization,
            dropout=dropout,
            kernel=kernel,
            key=key,
        )
        state = eqx.nn.State(ssm)
        return ssm, state
    elif model_name == "SpikingLinOSSwGeLU":
        if num_blocks is None:
            raise ValueError("Must specify num_blocks for LOSS.")
        if ssm_dim is None:
            raise ValueError("Must specify ssm_dim for LOSS.")
        ...
        state = eqx.nn.State(ssm)
        return ssm, state
    elif model_name == "SpikingLinOSSwBN":
        if num_blocks is None:
            raise ValueError("Must specify num_blocks for LOSS.")
        if ssm_dim is None:
            raise ValueError("Must specify ssm_dim for LOSS.")
        ...
        state = eqx.nn.State(ssm)
        return ssm, state
    elif model_name == "SpikingLinOSSwHeterogeneity":
        if num_blocks is None:
            raise ValueError("Must specify num_blocks for LOSS.")
        if ssm_dim is None:
            raise ValueError("Must specify ssm_dim for LOSS.")
        # if ssm_blocks is None:
        #     raise ValueError("Must specify ssm_blocks for LOSS.")
        ssm = SpikingLinOSSwHeterogeneity(
            num_blocks,
            data_dim,
            ssm_dim,
            hidden_dim,
